...
from utils.db import get_all_projects, save_hasil, execute_many, execute_query
...
# ------------------------------------------------------------
# POST /spk/hitung — Jalankan perhitungan metode SPK
# ------------------------------------------------------------
@spk_bp.route('/hitung', methods=['POST'])
def hitung():
    """
    Menjalankan algoritma SPK yang dipilih user.
    Body JSON: { "metode": "SAW" }
    """
    ...
    # Generate sesi_id unik untuk penyimpanan
    sesi_id = str(uuid.uuid4())

    # Simpan hasil ke database
    hasil_db = [
        {'project_id': h['project_id'], 'skor': h['skor'], 'ranking': h['ranking']}
        for h in hasil
    ]
...
def hitung_semua():
    """
    Menjalankan semua 4 metode SPK sekaligus untuk perbandingan.
    """
    projects = get_all_projects()
    ...
    if len(projects) < 2:
        return jsonify({
            'success': False,
            'message': 'Minimal 2 project diperlukan'
        }), 400
